...
float viewZValue = -3.0f;
float viewXValue = 0.0f;
float viewYValue = 0.0f;
float mixValue = 0.2f;
...
int main(void) {
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glm::mat4 view(1.0);
        view = glm::translate(view, glm::vec3(viewXValue, viewYValue, viewZValue));
        view = glm::rotate(view, glm::radians(-180.0f * static_cast<float>(cos(glfwGetTime() / 2.0f) + 0.5f)), glm::vec3(0.0f, 1.0f, 0.0f));
